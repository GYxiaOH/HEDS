...
train_layers = ['lossweight','conv1','conv2','conv3','conv4','conv5','conv6_10','fc7_10','fc8_10','conv6_200','fc7_200','fc8_200','conv6_1000','fc7_1000','fc8_1000','fc9']
...
source_validation_init_op = source_iterator.make_initializer(source_val_data.data)
...
#vec_class = tf.nn.relu(vec_class,name="vec_class")
...
# with tf.variable_scope("lossweight") as scope:
#     s_weight = tf.get_variable(name='s_weight',initializer=tf.random_uniform(minval=0,maxval=1,shape=[1]),dtype=tf.float32,constraint=constrain(s_weight))
#     t_weight = 1 - s_weight
...
weight_list = [v for v in var_list if v.name.split('/')[1] != 'biases:0' and v.name.split('/')[1] != 'bias:0']
...
with tf.Session() as sess:
    ...
    for epoch in range(num_epochs):
        ...
        for step in range(train_batches_per_epoch):
            source_img_batch,source_label_batch = sess.run(source_next_batch)
            target_img_batch, target_label_batch = sess.run(target_next_batch)

            _,loss = sess.run([train_op,loss_op],feed_dict={x_source:source_img_batch,
